...
from phone_agent.config.prompts_multi import (
    SYSTEM_PROMPT_MULTI as SYSTEM_PROMPT_MULTI_ZH,
    SYSTEM_PROMPT_MULTI_EN,
)
...
def get_system_prompt(lang: str = "cn", multi_strategy: bool = False) -> str:
    """
    Get system prompt by language.

    Args:
        lang: Language code, 'cn' for Chinese, 'en' for English.
        multi_strategy: If True, use multi-strategy prompt with id/text/image/point support.
    ...
    if multi_strategy:
        if lang == "en":
            return SYSTEM_PROMPT_MULTI_EN
        return SYSTEM_PROMPT_MULTI_ZH
